...
# GOLDEN cross-language PRG vector. Seed = 32 bytes of 0x01. The first four
# uniforms from prg_floats were generated by actually running the Python
# implementation; the SAME literals are hardcoded in edge-sdk/test/secureAgg
# so any divergence between the Python and TypeScript PRGs is caught.
GOLDEN_SEED = bytes([0x01] * 32)
GOLDEN_PRG_FLOATS = [
    0.8049047642420114,
    0.77533269603633,
    0.13199360046164554,
...
def test_prg_floats_matches_golden_vector():
    out = prg_floats(GOLDEN_SEED, 4)
    assert out.tolist() == GOLDEN_PRG_FLOATS, (
        "Python PRG diverged from the pinned golden cross-language vector"
    )


def test_seed_hex_round_trip():
    h = seed_to_hex(GOLDEN_SEED)
    assert h == "01" * 32
    assert len(h) == 64
    assert hex_to_seed(h) == GOLDEN_SEED


def test_prg_floats_in_unit_interval_and_deterministic():
    a = prg_floats(GOLDEN_SEED, 16)
    b = prg_floats(GOLDEN_SEED, 16)
    assert (a == b).all()
